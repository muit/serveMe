...
Routes.add = function(url, callback)
{
  ...
  Routes._hashIds[url] = {callback: callback, method: undefined};
};

Routes.get = function(url, callback)
{
  if(typeof url != "string")
    throw new IncorrectArgumentType("Routes.reset: Url must be a string, not a "+typeof url+".");

  Routes._hashIds[url] = {callback: callback, method: "GET"};
};

Routes.post = function(url, callback)
{
  if(typeof url != "string")
    throw new IncorrectArgumentType("Routes.reset: Url must be a string, not a "+typeof url+".");

  Routes._hashIds[url] = {callback: callback, method: "POST"};
};


Routes.take = function(url)
{
  if(typeof url != "string")
    throw new IncorrectArgumentType("Routes.reset: Url must be a string, not a "+typeof url+".");
