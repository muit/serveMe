...
Routes.prototype = {
  ...
  take: function(method, url, params) {
    ...
    if (hash.data) {
      //return a route
      route.callback = hash.data[method];
    }
  ...
  serve: function(url, request, response){
    var route = this.core.routes.take(request.method, url.pathname, url.query);

    if (route.callback) {
      request.params = route.params;
      var data = route.callback(request, function(data){
        next(response, data);
      }, response);
      next(response, data);
      return true;
