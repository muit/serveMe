...
HttpServer.prototype.serveError = function(status, request, response)
{
    ...
    if(this.options.debug || this.files[path] === undefined)
    {
        fs.readFile(path, function(err, data)
        {
            if (err)
            {
                writeResponse(500, {pathname: ""}, "ERROR: 500 - Internal server error.", request, response);
            ...
            writeResponse(status, {pathname: path}, data, request, response);
        });
    }
    else
        writeResponse(status, {pathname: path}, this.files[path], request, response);
};

/**
 * @api private
 */
function writeResponse(status, url, data, request, response)
{
    var session,
        type = mime.get(url.pathname.split(".").pop()),
        head = {
            'Content-Length': data.length,
            'Content-Type': type
        };
