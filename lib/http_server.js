...
HttpServer.prototype.serveRoute = function(url, request, response){
    if(ServeMe.Routes)
    {
        var route = ServeMe.Routes.take(url.pathname);
        if(route && route.callback)
        {
            if(!route.method || route.method == request.method)
            {
                var data = route.callback(request, response);
...
function writeResponse(status, url, data, request, response, file)
{
    ...
    var result = ServeMe.Session.lookupOrCreate(url, request);
    if(result.session) {
        head['Set-Cookie'] = result.session.getSetCookieHeaderValue();
        if(result.data)
        {
