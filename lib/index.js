...
function ServeMe(options, port){
    ...
    var self = this;
    this.Routes.add("/favicon.ico", function(request, response)
    {
        return self.server.get_file_data(self.config.favicon, request, response);
    });
}
...
ServeMe.prototype =
{
    ...
    start: function(port){
        if(this.config.cluster.enabled)
            return new ClusterServer(this, port || this.port);
        else
